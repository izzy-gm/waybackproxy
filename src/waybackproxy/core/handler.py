...
"""HTTP request handler for Wayback proxy."""
...
try:
	...
	print('WaybackProxy now requires urllib3 to be installed.')
	sys.exit(1)

# Configuration globals
# These are set by __main__.py from Settings dataclasses
# Default values match ProxySettings defaults
LISTEN_PORT = 8888
DATE_TOLERANCE = 365
GEOCITIES_FIX = True
QUICK_IMAGES = True
...
from .cache import SharedState

# Create singleton shared state instance
shared_state = SharedState()
shared_state.load_whitelist()

# UI controller reference (set by __main__.py if UI is enabled)
# This allows web interface to sync LCD display when date changes
ui_controller = None
...
class Handler(socketserver.BaseRequestHandler):
	...
	def drain_conn(self, conn):
		"""Safely drain connection with fallback for urllib3 versions lacking drain_conn().

		Some urllib3 versions (1.25, 1.26.19) don't have drain_conn() method.
		Falls back to read() which accomplishes the same goal.
		"""
		getattr(conn, 'drain_conn', conn.read)()

	def sanitize_redirect(self, url):
		"""Sanitize redirect URLs to ensure HTTP protocol and proper formatting.

		Converts HTTPS URLs to HTTP (archived HTTPS content is often inaccessible)
		and adds missing protocol to protocol-relative URLs.

		Args:
			url: The redirect destination URL

		Returns:
			Sanitized URL with HTTP protocol
		"""
		if url[0] != '/' and '://' not in url:
			# Protocol-relative URL, add http://
			return 'http://' + url
		elif url[:8].lower() == 'https://':
			# Convert HTTPS to HTTP for archive compatibility
			return 'http://' + url[8:]
		else:
			return url

	def is_asset_request(self, url):
		"""Determine if this is an asset request (image, CSS, JS) vs HTML.

		Args:
			url: The URL to check

		Returns:
			True if this appears to be a static asset, False otherwise
		"""
		# Check file extension
		return bool(re.search(r'\.(jpe?g|png|gif|css|js|ico|svg|webp|bmp|woff2?|ttf|eot)(\?|$)', url, re.I))
	...
	def handle(self):
		...
		if len(split) < 2 or split[0].upper() != 'GET':
			# only GET is implemented (accept lowercase for Netscape Communicator compatibility)
		...
		try:
			if path in pac_file_paths:
				...
				if path[:5] != '/web/':
					...
					# Detect if this is an asset (has im_, js_, cs_ tag)
					is_quick_asset = effective_date.endswith(('im_', 'js_', 'cs_'))
					if is_quick_asset:
						_print('[>] [QI-asset]', archived_url)
					else:
			...
			# Determine if this is an asset request for optimization
			is_asset = self.is_asset_request(archived_url) or (
				hostname == 'web.archive.org' and
				effective_date.endswith(('im_', 'js_', 'cs_'))
			)

			# Define retry strategies based on content type
			if is_asset:
				# Assets: Fail fast with minimal retries
				# Reduces wasted time on missing images from 15s to 1.5s
				retry = urllib3.util.retry.Retry(
					total=2, connect=2, read=1, redirect=0,
					backoff_factor=0.3,
					status_forcelist=[500, 502, 503, 504]
				)
			else:
				# HTML: Conservative retries for transient server errors
				retry = urllib3.util.retry.Retry(
					total=5, connect=3, read=2, redirect=0,
					backoff_factor=0.5,
					status_forcelist=[500, 502, 503, 504]
				)
			...
			if split[2] == 'web.archive.org':
				...
				if new_url:
					...
					request_url = new_url
				elif WAYBACK_API and not (QUICK_IMAGES and is_asset):
					# Not in cache => contact API (skip for QUICK_IMAGES assets)
					try:
						...
						# Use explicit timeout with separate connect/read values
						availability = json.loads(
							self.shared_state.http.request(
								'GET', availability_endpoint,
								timeout=urllib3.Timeout(connect=5.0, read=10.0),
								retries=urllib3.util.retry.Retry(total=1, connect=1, read=0, backoff_factor=0)
							).data
						)
						closest = availability.get('archived_snapshots', {}).get('closest', {})
						new_date = closest.get('timestamp', None)
					except Exception as e:
						_print(f'[!] Failed to fetch Wayback availability data: {e}')
						new_date = None
			...
			# Start fetching the URL with explicit timeout
			# Archive.org requires generous timeouts: 10s connect, 60s read
			while True:
				conn = self.shared_state.http.urlopen(
					'GET', request_url,
					redirect=False,
					retries=retry,
				...
				if destination:
					...
					if match:
						# Sanitize redirect to ensure HTTP protocol and proper formatting
						archived_dest = self.sanitize_redirect(match.group(2))

						# Remove extraneous :80 from URL for comparison
						archived_dest_normalized = re.sub('''^([^/]*//[^/]+):80''', '\\1', archived_dest)

						# Check if the archived URL is different.
						if archived_dest_normalized != archived_url:
		...
		if 'text/html' in guessed_content_type:
			...
			if mode == 0: # Wayback Machine
				...
				if b'<title>Wayback Machine</title>' in data:
					...
					match = re.search(rb'''<iframe id="playback" src="((?:(?:https?:)?//web.archive.org)?/web/[^"]+)"''', data)
					if match:
						# Extract the content URL.
						request_url = match.group(1).decode('ascii', 'ignore')
						archived_url = '/'.join(request_url.split('/')[5:])

						# Start fetching the URL with timeout
				...
				if b'<title></title>' in data and b'<span class="label style-scope media-button"><!---->Wayback Machine<!----></span>' in data:
					match = re.search(rb'''<p class="impatient"><a href="(?:(?:https?:)?//web\.archive\.org)?/web/([^/]+)/([^"]+)">Impatient\?</a></p>''', data)
					if match:
						# Sanitize the URL.
						...
						match = re.search(rb'''<p class="code shift red">Got an HTTP ([0-9]+)''', data)
						try:
				...
				data = re.sub(rb'''<script (?:type="text/javascript" )?src="(?:https?:)?//(?:web-static\.)?archive\.org/_static/js/.*<!-- End Wayback Rewrite JS Include -->\r?\n''', b'', data, count=1, flags=re.S)
				# Remove toolbar. The if_ asset tag serves no toolbar, but we remove it just in case.
				data = re.sub(rb'''<!-- BEGIN WAYBACK TOOLBAR INSERT -->.*<!-- END WAYBACK TOOLBAR INSERT -->''', b'', data, count=1, flags=re.S)
				# Remove comments on footer.
				data = re.sub(rb'''<!--\r?\n     FILE ARCHIVED .*$''', b'', data, flags=re.S)
				# Fix base tag.
				data = re.sub(rb'''(<base\s+[^>]*href=["']?)(?:(?:https?:)?//web.archive.org)?/web/[^/]+/(?:[^:/]+://)?''', rb'\1http://', data, flags=re.I + re.S)
				...
				if QUICK_IMAGES:
					...
					def add_to_date_cache(match):
						...
						return orig_url
					data = re.sub(rb'''(?:(?:https?:)?//web.archive.org)?/web/([^/]+)/([^"'#<>]+)''', add_to_date_cache, data)
				...
				data = re.sub(rb'''<style> \n.zoomout { -webkit-transition: .*$''', b'', data, flags=re.S)
				# Remove clearly labeled snippets from Geocities.
				data = re.sub(rb'''^.*<!-- text above generated by server\. PLEASE REMOVE -->''', b'', data, flags=re.S)
				data = re.sub(rb'''<!-- following code added by server\. PLEASE REMOVE -->.*<!-- preceding code added by server\. PLEASE REMOVE -->''', b'', data, flags=re.S)
				data = re.sub(rb'''<!-- text below generated by server\. PLEASE REMOVE -->.*$''', b'', data, flags=re.S)

				# Fix links.
				data = re.sub(rb'''//([^\.]*\.)?oocities\.com/''', rb'//\1geocities.com/', data, flags=re.S)
	...
	def send_passthrough(self, conn, http_version, content_type, request_url):
		...
		try:
			...
			# Larger chunks = fewer Python iterations and TCP sends
			content_length = conn.headers.get('Content-Length')
			if content_length and int(content_length) > 102400:  # >100KB
				chunk_size = 32768  # 32KB chunks for large files
			else:
				chunk_size = 16384  # 16KB chunks for smaller files

			for data in conn.stream(chunk_size):
				self.request.sendall(data)
		finally:
			# Always release connection back to pool, even if client disconnects
			conn.release_conn()
			self.request.close()
	...
	def send_error_page(self, http_version, code, reason):
		...
		error_page_len = len(error_page.encode('utf-8'))
	...
	def send_redirect_page(self, http_version, target, code=302):
		...
		self.request.sendall('{0} {1} Found\r\nLocation: {2}\r\nContent-Type: text/html\r\nContent-Length: {3}\r\n\r\n{4}'.format(http_version, code, target, len(redirectpage.encode('utf-8')), redirectpage).encode('utf8', 'ignore'))
		self.request.close()
